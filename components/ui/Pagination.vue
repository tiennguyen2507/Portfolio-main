<template>
  ...
  >
    <div v-if="showInfo && hasItemInfo" class="text-sm text-gray-600 whitespace-nowrap">
      Hiển thị
    ...
    <div class="inline-flex items-center gap-3">
      <button
        type="button"
        class="h-8 w-8 inline-flex items-center justify-center rounded-md text-gray-400 hover:text-gray-700 hover:bg-gray-50 disabled:opacity-40 disabled:pointer-events-none focus:outline-none focus-visible:outline-none focus:ring-0"
      ...
      <div class="inline-flex items-center gap-2">
        <span class="px-2.5 py-1 rounded-md border border-gray-200 bg-white text-gray-900 text-sm font-medium shadow-sm min-w-[2rem] text-center">{{ page }}</span>
        <span class="text-gray-400">/</span>
        <span class="text-gray-700 text-sm">{{ totalPages }}</span>
      </div>
      ...
      >
        <svg class="h-4 w-4" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
          <path fill-rule="evenodd" d="M7.293 4.293a1 1 0 011.414 0l5 5a1 1 0 010 1.414l-5 5a1 1 0 11-1.414-1.414L10.586 10 7.293 6.707a1 1 0 010-1.414z" clip-rule="evenodd" />
        </svg>
      </button>
    </div>
  </div>
...
const containerClass = computed(() =>
  (props.split
    ...
    : 'inline-flex items-center gap-4 select-none') + ' focus:outline-none focus-visible:outline-none focus:ring-0'
);
